...
import { allProductSlugsQuery, getProductBySlug } from "@/lib/queries";
import type { SanityImage } from "@/types/product";
import { urlFor } from "@/lib/sanity";
import { getWhatsAppInquiryUrl } from "@/lib/whatsapp";
import { WhatsAppGlyph } from "@/components/WhatsAppGlyph";
import { InquireNowButton } from "@/components/InquiryModal";
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const whatsappMessage = `Hi, I'm interested in the product "${product.title}".`;
  const whatsappHref1 = getWhatsAppInquiryUrl(
    whatsappMessage,
    process.env.NEXT_PUBLIC_WHATSAPP_NUMBER_1,
  );
  const whatsappHref2 = getWhatsAppInquiryUrl(
    whatsappMessage,
      ...
      <div className="mx-auto min-w-0 max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="grid min-w-0 grid-cols-1 gap-8 sm:gap-12 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)] lg:items-start">
          ...
          <div className="space-y-5 sm:space-y-6">
            ...
            <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:flex-wrap sm:items-start">
              {brochureUrl ? (
                <a
                  href={brochureUrl}
                  target="_blank"
              ...
              <div className="flex w-full flex-col gap-3 sm:w-auto">
                ...
                {whatsappHref2 ? (
                  ...
                  >
                    <WhatsAppGlyph className="h-4 w-4 shrink-0" />
                    Inquire Through WhatsApp 2
                  </a>
                ) : null}
                <InquireNowButton productTitle={product.title} />
              </div>
            </div>
